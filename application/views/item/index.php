...
<body>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="card-title">
                                <h4>Daftar <?= $judul; ?></h4>
                                <button class="btn btn-info" data-toggle="modal" data-target="#item_add">Tambah <?= $judul; ?></button>
                            </div>
                                        ...
                                        <?php foreach ($data as $row) : ?>
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    <a href="javascript:;" onclick="deleteItem('<?= $row['id'] ?>')" class="btn btn-danger">Hapus</a>
                                                </td>
                                            </tr>
                                        <?php endforeach; ?>
                                    </tbody>
            ...
            <div class="modal" tabindex="-1" id="item_add">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form method="post" action="<?= site_url('item/add'); ?>">
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="item_name">Item Name</label>
                                    <input type="text" class="form-control" id="item_name" name="item_name" value="<?= set_value('item_name'); ?>" required>
                                </div>
    ...
    <script>
        function deleteItem(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '<?= base_url('item/delete/') ?>' + id + '?_csrf=' + '<?= $this->security->get_csrf_hash() ?>';
                }
            })
        }
        ...
        function editModal(id) {
            ...
            $.ajax({
                url: '<?= base_url('item/get_item/') ?>' + id + '?<?= $this->security->get_csrf_token_name(); ?>=' + <?= $this->security->get_csrf_token_name(); ?>,
                type: 'GET',
                dataType: 'json',
                success: function(data) {
                    ...
                    // change action
                    $('form').attr('action', '<?= base_url('item/update') ?>');
                }
            });
        }

        //create everytime modal hide clear form
        $('#item_add').on('hidden.bs.modal', function(e) {
            $('#item_name').val('');
            $('#id').val('');
            // change title
            ...
            // change action
            $('form').attr('action', '<?= base_url('item/add') ?>');
        });
