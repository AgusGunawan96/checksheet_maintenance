...
<body>
    ...
    <div class="wrapper">
        ...
        <div class="page-wrapper">
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="card-title">
                                <h4>Daftar <?= $judul; ?></h4>
                                <button class="btn btn-info" data-toggle="modal" data-target="#method_add">Tambah <?= $judul; ?></button>
                            </div>
            ...
            <div class="modal" tabindex="-1" id="method_add">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form method="post" action="<?= site_url('method/add'); ?>">
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="method_name">Method Name</label>
                                    <input type="text" class="form-control" id="method_name" name="method_name" value="<?= set_value('method_name'); ?>" required>
                                </div>
    ...
    <script>
        function deleteMethod(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '<?= base_url('method/delete/') ?>' + id + '?_csrf=' + '<?= $this->security->get_csrf_hash() ?>';
                }
            })
        }
        ...
        function editModal(id) {
            ...
            $.ajax({
                url: '<?= base_url('method/get_method/') ?>' + id + '?<?= $this->security->get_csrf_token_name(); ?>=' + <?= $this->security->get_csrf_token_name(); ?>,
                type: 'GET',
                dataType: 'json',
                success: function(data) {
                    $('#method_add').modal('show');
                    ...
                    // change action
                    $('form').attr('action', '<?= base_url('method/update') ?>');
                }
            });
        }

        //create everytime modal hide clear form
        $('#method_add').on('hidden.bs.modal', function(e) {
            $('#method_name').val('');
            $('#id').val('');
            // change title
            $('.modal-title').text('Tambah Method');
            // change button
            $('.modal-footer button[type=submit]').text('Submit');
            // change action
            $('form').attr('action', '<?= base_url('method/add') ?>');
        });
