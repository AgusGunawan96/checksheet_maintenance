...
<body>
                    ...
                    <div class="card">
                        <div class="card-body">
                            <div class="card-title">
                                <h4>Daftar <?= $judul; ?></h4>
                                <button class="btn btn-info" data-toggle="modal" data-target="#part_add">Tambah <?= $judul; ?></button>
                            </div>
            ...
            <div class="modal" tabindex="-1" id="part_add">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <form method="post" action="<?= site_url('part/add'); ?>">
                            <div class="modal-body">
                                <div class="form-group">
                                    <label for="part_name">Part Name</label>
                                    <input type="text" class="form-control" id="part_name" name="part_name" value="<?= set_value('part_name'); ?>" required>
                                </div>
    ...
    <script>
        function deleteInspectionPart(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = '<?= base_url('part/delete/') ?>' + id + '?_csrf=' + '<?= $this->security->get_csrf_hash() ?>';
                }
            })
        }
        ...
        function editModal(id) {
            ...
            $.ajax({
                url: '<?= base_url('part/get_part/') ?>' + id + '?<?= $this->security->get_csrf_token_name(); ?>=' + <?= $this->security->get_csrf_token_name(); ?>,
                type: 'GET',
                dataType: 'json',
                success: function(data) {
                    ...
                    // change action
                    $('form').attr('action', '<?= base_url('part/update') ?>');
                }
            });
        }

        //create everytime modal hide clear form
        $('#part_add').on('hidden.bs.modal', function(e) {
            $('#part_name').val('');
            $('#rank').val('');
            $('#id').val('');
            // change title
            ...
            // change action
            $('form').attr('action', '<?= base_url('part/add') ?>');
        });
