<div class="table-responsive" style="height: 400px !important">
    ...
    foreach ($data as $row) {
        ...
        // Measure Data input - cek apakah determination standard mengandung angka
        $measureDataValue = $row['measure_data'];
        if (!preg_match('/[0-9]/', $row['determination_standard'])) {
            $measureDataValue = 'No Abnormality';
        }
        // echo '<td><input type="text" class="form-control form-control-sm measure-data" data-id="' . $row['id'] . '" value="' . htmlspecialchars($measureDataValue) . '"></td>'; 
        echo '<td><input type="text" class="form-control form-control-sm measure-data" data-id="' . $row['id'] . '" data-determination="' . htmlspecialchars($row['determination_standard']) . '" value="' . htmlspecialchars($measureDataValue) . '"></td>';

        // Judgement input atau select
        $isNumeric = preg_match('/[0-9]/', $row['determination_standard']);
        if ($isNumeric) {
            // Jika determination standard mengandung angka, tampilkan input teks
            // echo '<td><input type="text" class="form-control form-control-sm judgement" data-id="' . $row['id'] . '" value="' . htmlspecialchars($row['judgement']) . '"></td>';
            echo '<td><input type="text" class="form-control form-control-sm judgement" data-id="' . $row['id'] . '" value="' . htmlspecialchars($row['judgement'])  . '"></td>';

        } else {
            // Jika tidak, tampilkan dropdown
            echo '<td>
                    <select class="form-control form-control-sm judgement" data-id="' . $row['id'] . '">
                        ...
                        <option value="Repaired Fix" ' . ($row['judgement'] == 'Repaired Fix' ? ' selected' : '') . '>&#8855;</option> 
                    </select>
                  </td>';
        }  
        // Measure input
    ...
    $(document).ready(function() {
        ...
        // Fungsi untuk menentukan judgment berdasarkan determination standard dan measure data
        function calculateJudgment(determinationStandard, measureData) {
            var match = determinationStandard.match(/([\d.]+)\s*±\s*([\d.]+)/);
            if (!match) return '';

            var nominal = parseFloat(match[1]);
            var tolerance = parseFloat(match[2]);
            var upperLimit = nominal + tolerance;
            var lowerLimit = nominal - tolerance;

            if (measureData > upperLimit) {
                return 'Abnormal';
            } else if (measureData < lowerLimit) {
        ...
        // Saat measure data diubah, perbarui judgment
        $('.measure-data').on('input', function() {
            var id = $(this).data('id');
            var measureData = parseFloat($(this).val());
            var determinationStandard = $('.determination-standard[data-id="' + id + '"]').text();

            if (isNaN(measureData) || !determinationStandard) {
                $('.judgement[data-id="' + id + '"]').val('');
                return;
            }

            var judgment = calculateJudgment(determinationStandard, measureData);
            $('.judgement[data-id="' + id + '"]').val(judgment);
        });

        // Saat judgment diubah secara manual, perbarui measure data
        $('.judgement').on('change', function() {
            var id = $(this).data('id');
            var judgement = $(this).val();
            if (judgement === 'No Abnormality') {
                $('.measure-data[data-id="' + id + '"]').val('No Abnormality');
            } else if (judgement === 'Abnormal') {
                $('.measure-data[data-id="' + id + '"]').val('Abnormal');
            } else if (judgement === 'Cautious') {
                $('.measure-data[data-id="' + id + '"]').val('Cautious');
            } else if (judgement === 'Repaired Fix') { // Added condition for "Repaired"
                $('.measure-data[data-id="' + id + '"]').val('Repaired Fix');
            } else {
                $('.measure-data[data-id="' + id + '"]').val('');
            }
        });
        ...
        $('#saveButton').on('click', function() {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    requestData[csrfName] = csrfHash; // Sertakan token CSRF dalam permintaan
    ...
    <!-- <script>
        $(document).ready(function() {
            ...
            // Saat judgement diubah, perbarui measure data
            $('.judgement').on('change', function() {
                var id = $(this).data('id');
                var judgement = $(this).val();
                if (judgement === 'No Abnormality') {
